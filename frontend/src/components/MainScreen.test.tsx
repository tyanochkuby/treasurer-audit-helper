...
describe('MainScreen', () => {
  it('keeps the selected document identity in a sticky header', () => {
    ...
    expect(screen.getByRole('button', { name: 'Odśwież' })).toHaveClass('w-10', 'p-0', 'xl:w-auto')
    expect(screen.getByRole('button', { name: 'Eksportuj CSV' })).toHaveClass('w-10', 'p-0', 'xl:w-auto')
    expect(screen.getByText('Odśwież')).toHaveClass('hidden', 'xl:inline')
    expect(screen.getByText('Eksportuj CSV')).toHaveClass('hidden', 'xl:inline')
  ...
  it('offers one reactive expand/collapse-all control for multi-event histories', async () => {
    ...
    expect(expandAll).toHaveClass('w-10', 'p-0', 'xl:w-auto', 'xl:px-3')
    expect(screen.getByText('Rozwiń wszystkie')).toHaveClass('hidden', 'xl:inline')

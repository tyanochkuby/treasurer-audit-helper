...
describe('ContractSidebar', () => {
  it('shows a compact organization id while retaining the full value as a tooltip', () => {
    render(<ContractSidebar contracts={[contract]} selectedId="" open onClose={vi.fn()} onSelect={vi.fn()} />)
  ...
  it('keeps the search header outside the independently scrollable contract list', () => {
    const { container } = render(<ContractSidebar contracts={[contract]} selectedId="" open onClose={vi.fn()} onSelect={vi.fn()} />)

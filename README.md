...
The contract selector includes each contract's unfiltered audit-event count. Counts for all active contracts are computed in one set-based query: document and related-entity relationships are materialized once, audit rows are matched by `EntityId`/`ParentId`, and the result is grouped by root contract. This avoids an N+1 history query per contract. On the inspected database, all 1,005 counts were produced in about 0.9 seconds of query execution and under 2 seconds for the complete repository call including a new connection.
